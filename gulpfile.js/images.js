...
// Watch Tasks
gulp.task('minImages', (cb) => {
  gulp.watch(paths.imageFilesGlob, gulp.series(minimizeImages, convertToWebP, 'reload'));
});
gulp.task('minPostImages', (cb) => {
  gulp.watch(paths.postImageFilesGlob, gulp.series(minimizePostImages, 'reload'));
});
gulp.task('watchImages', gulp.parallel('minImages', 'minPostImages'));